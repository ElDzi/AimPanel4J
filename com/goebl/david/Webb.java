...
/**
 * Lightweight Java HTTP-Client for calling JSON REST-Services (especially for
 * Android).
 *
 * @author hgoebl
 */
public class Webb {
	...
	/**
	 * Create an instance which can be reused for multiple requests in the same
	 * Thread.
	 * 
	...
	/**
	 * Set the value for a named header which is valid for all requests in the
	 * running JVM. <br>
	 * The value can be overwritten by calling
	 * {@link Webb#setDefaultHeader(String, Object)} and/or
	 * {@link com.goebl.david.Request#header(String, Object)}. <br>
	 * For the supported types for values see
	 * {@link Request#header(String, Object)}.
	 *
	 * @param name
	 *            name of the header (regarding HTTP it is not case-sensitive,
	 *            but here case is important).
	 * @param value
	 *            value of the header. If <code>null</code> the header value is
	 *            cleared (effectively not set).
	 *
	 ...
	 * For all requests this value is taken as a kind of prefix for the
	 * effective URI, so you can address the URIs relatively. The value is only
	 * taken when {@link Webb#setBaseUri(String)} is not called or called with
	 * <code>null</code>.
	 *
	 * @param globalBaseUri
	...
	/**
	 * The number of characters to indent child properties, <code>-1</code> for
	 * "productive" code. <br>
	 * Default is production ready JSON (-1) means no indentation (single-line
	 * serialization).
	 * 
	 * @param indentFactor
	 ...
	 * In contrast to {@link java.net.HttpURLConnection}, we use a default
	 * timeout of 10 seconds, since no timeout is odd.<br>
	 * Can be overwritten for each Request with
	 * {@link com.goebl.david.Request#connectTimeout(int)}.
	 * 
	 * @param globalConnectTimeout
	 *            the new timeout or <code>&lt;= 0</code> to use
	 *            HttpURLConnection default timeout.
	 */
	 ...
	 * <br>
	 * In contrast to {@link java.net.HttpURLConnection}, we use a default
	 * timeout of 3 minutes, since no timeout is odd.<br>
	 * Can be overwritten for each Request with
	 * {@link com.goebl.david.Request#readTimeout(int)}.
	 * 
	 ...
	 * </a>. <br>
	 * Use this method to set the behaviour for all requests created by this
	 * instance when receiving redirect responses. You can overwrite the setting
	 * for a single request by calling {@link Request#followRedirects(boolean)}.
	 * 
	 * @param auto
	...
	/**
	 * Set a custom {@link javax.net.ssl.SSLSocketFactory}, most likely to relax
	...
	/**
	 * Set a custom {@link javax.net.ssl.HostnameVerifier}, most likely to relax
	 * host-name checking.
	 * 
	 ...
	 * For all requests this value is taken as a kind of prefix for the
	 * effective URI, so you can address the URIs relatively. The value takes
	 * precedence over the value set in {@link #setGlobalBaseUri(String)}.
	 *
	 * @param baseUri
	 *            the prefix for all URIs of new Requests.
	...
	/**
	 * Set the value for a named header which is valid for all requests created
	 * by this instance. <br>
	 * The value takes precedence over
	 * {@link Webb#setGlobalHeader(String, Object)} but can be overwritten by
	 * {@link com.goebl.david.Request#header(String, Object)}. <br>
	 * For the supported types for values see
	 * {@link Request#header(String, Object)}.
	 *
	 ...
	 * @param value
	 *            value of the header. If <code>null</code> the header value is
	 *            cleared (effectively not set). When setting the value to null,
	 *            a value from global headers can shine through.
	 *
	 ...
	 * 
	 * @param retryManager
	 *            the new manager for deciding whether it makes sense to retry a
	 *            request.
	 */
	...
	/**
	 * Creates a <b>GET HTTP</b> request with the specified absolute or relative
	 * URI.
	 * 
	 * @param pathOrUri
	 *            the URI (will be concatenated with global URI or default URI
	 *            without further checking). If it starts already with http://
	 *            or https:// this URI is taken and all base URIs are ignored.
	 * @return the created Request object (in fact it's more a builder than a
	 *         real request object)
	 */
	...
	/**
	 * Creates a <b>POST</b> HTTP request with the specified absolute or
	 * relative URI.
	 * 
	 * @param pathOrUri
	 *            the URI (will be concatenated with global URI or default URI
	 *            without further checking) If it starts already with http:// or
	 *            https:// this URI is taken and all base URIs are ignored.
	 * @return the created Request object (in fact it's more a builder than a
	 *         real request object)
	 */
	...
	/**
	 * Creates a <b>PUT</b> HTTP request with the specified absolute or relative
	 * URI.
	 * 
	 * @param pathOrUri
	 *            the URI (will be concatenated with global URI or default URI
	...
	/**
	 * Creates a <b>DELETE</b> HTTP request with the specified absolute or
	 * relative URI.
	 * 
	 * @param pathOrUri
	 *            the URI (will be concatenated with global URI or default URI
		...
		try {
			...
			if (request.method != Request.Method.GET && request.method != Request.Method.DELETE) {
				if (request.streamPayload) {
					...
					byte[] requestBody = WebbUtils.getPayloadAsBytesAndSetContentType(connection, request,
	...
	private void writeBody(HttpURLConnection connection, byte[] body) throws IOException {
		...
		// "E/StrictMode﹕ A resource was acquired at attached stack trace but
		// never released"
		// It seems like some kind of bug in special devices (e.g. 4.0.4/Sony)
		// but does not
		// happen e.g. on 4.4.2/Moto G.
		// Closing the stream in the try block might help sometimes (it's
		// intermittently),
		// but I don't want to deal with the IOException which can be thrown in
		// close().
	...
	private static class AutoDisconnectInputStream extends FilterInputStream {
		...
		/**
		 * Creates an <code>AutoDisconnectInputStream</code> by assigning the
		 * argument <code>in</code> to the field <code>this.in</code> so as to
